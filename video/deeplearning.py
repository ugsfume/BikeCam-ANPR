...
net = cv2.dnn.readNetFromONNX('./static/model/plate_detect.onnx')
...
def non_maximum_supression(input_image,detections):
    # x, y, w , h, confidence, probability
    boxes = []
    confidences = []
...
def video_anpr(path):
    ...
    while True:
        ...
        if frame_counter % 25 == 0:     # 25 frame per second
            _, text_list = yolo_predictions(frame, net)
            
            license_plate.extend(text_list)

            for i in range(len(text_list)):
                timestamps.append(frame_counter/fps)
    
    zipped_list = list(zip(license_plate,timestamps))
    cap.release()
    return zipped_list